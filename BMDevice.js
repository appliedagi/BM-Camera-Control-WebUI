...
class BMDevice {
  ...
  constructor(hostname, secure = false) {
    ...
    this.APIAddress =
      (this.useHTTPS ? "https://" : "http://") + hostname + "/control/api/v1";
    this.name = this.hostname.replace(".local", "").replaceAll("-", " ");

    // Initialize WebSocket
    this.ws = new WebSocket(
      (this.useHTTPS ? "wss://" : "ws://") +
        hostname +
        "/control/api/v1/event/websocket"
    );
    ...
    this.ws.onopen = (event) => {
      ...
      self.ws.send(
        JSON.stringify({ type: "request", data: { action: "listProperties" } })
      );
      ...
      sleep(100).then(() => {
        ...
        this.availableProperties.forEach((str) => {
          self.ws.send(
            JSON.stringify({
              type: "request",
              data: { action: "subscribe", properties: [str] },
            })
          );
        });
      });
  ...
  seek(direction) {
    ...
    clips.forEach((clip) => {
      if (
        runningSum + clip.frameCount > playbackData.position &&
        !currentClipFound
      ) {
        currentClipIndex = i;
    ...
    let newClipIndex = Math.min(
      Math.max(0, direction ? currentClipIndex + 1 : currentClipIndex - 1),
      clips.length - 1
    );
